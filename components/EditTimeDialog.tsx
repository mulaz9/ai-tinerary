...
export default function EditTimeDialog({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (startMin === null) {
      setError("Inserisci un orario di inizio valido (HH:MM).");
      return;
    }
    if (end) {
      const endMin = timeToMinutes(end);
      if (endMin === null) {
        setError("Inserisci un orario di fine valido (HH:MM).");
        return;
      }
      if (endMin <= startMin) {
      ...
      >
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          <p className="text-[11px] text-white/40">
            Le attività del giorno verranno riordinate automaticamente in base
            al nuovo orario di inizio.
          </p>
